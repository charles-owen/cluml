...
<li><a class="tabs-right"><span class="icons-cl icons-cl-arrowthick-1-e"></span>Move Right</a></li>
    ...
    const del = () => {
        let current = main.tabs.currentDiagram();
        if(current === null) {
            return;
        }
    ...
    this.activate = function() {
	    ...
	    menu.click('.tabs-left', () => {
		    ...
		    if(main.model.diagrams.moveLeft(tabs.active)) {
				tabs.sync();
		    }
	    });
	    ...
	    menu.click('.tabs-right', () => {
		    ...
		    if(main.model.diagrams.moveRight(tabs.active)) {
			    tabs.sync();
		    }
	    });
    }
    ...
    this.opening = function() {
    	const tabs = main.tabs;
		const diagrams = main.model.diagrams;

	    menu.enable('.tabs-delete', diagrams.canDelete(tabs.active));
        menu.enable('.tabs-left', diagrams.canMoveLeft(tabs.active));
        menu.enable('.tabs-right', diagrams.canMoveRight(tabs.active));
    }
