...
export const Tabs = function(main) {
    ...
    this.sync = function() {
        ...
        collection.forEach((diagram) => {
            ...
            tabsNew.push({li: li, pane: pane, diagram: diagram, view: view});
        });
    ...
    function needSync() {
        ...
        if(tabs.length !== collection.length) {
            // If have different number of diagrams than tabs
            return true;
        }
    ...
    this.validateName = function(name, skip) {
        var diagrams = main.model.diagrams;
        var collection = diagrams.getDiagrams();
